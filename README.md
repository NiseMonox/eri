...
- 💬 **对话式秘书**:「手头有点事,下午再去」→ 自动改期并再催;「那我去健身咯」→ 记录今日完成;无回应自动追催到底
...
- 🧠 **LLM 分层**:正则快路径零延迟;复杂语句用 `claude -p`(订阅内)带上下文决策(开放事项 + 近期对话),DeepSeek 可切换;LLM 失败一律降级不影响核心提醒
...
├── bot/          # Telegram long-polling + 正则快路径
├── llm/          # claude -p / DeepSeek 抽象
