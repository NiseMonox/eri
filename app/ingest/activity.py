...
Body: {"date": "2026-08-29"(JST 日期,省略=昨天), "steps": 8432, "active_kcal": 512.3,
       "exercise_min": 42, "distance_km": 5.2} —— 指标全部可选,但至少要有一个。
同一天重复上报覆盖更新(取最后一次),所以自动化跑几次都无所谓。"""
...
# 整串里只允许恰好一个数字(千分位/前后单位文字 OK),数字之外不得再有数字——
# 从而 "1e9"/"5.2e3" 这类指数写法、多数字串都会被整体拒绝而不是抽错值
_NUM_FULL = re.compile(r"^\D*?([-+]?\d[\d,]*(?:\.\d+)?)\D*$")
...
def ingest(payload: dict) -> dict:
    ...
    if not date:
        date = (clock.now_local() - timedelta(days=1)).strftime("%Y-%m-%d")
    try:
    ...
    for field, (metric, unit, lo, hi) in FIELDS.items():
        ...
        if not lo <= val <= hi:
            ...
            continue
        body_metrics.upsert(metric, round(val, 2), at, unit=unit, source="shortcuts")
        saved[metric] = val
    if errors:
